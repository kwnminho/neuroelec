...
prog_uint16_t CIEL12[] PROGMEM = { 
        ...
        3284,3320,3356,3393,3430,3467,3504,3542,3579,3617,3656,3694,3733,3773,3812,3852,3892,3932,3973,4013,4055,4096};
/*
...
{
  Wire.beginTransmission(_pca9685_address);
  Wire.send(0x06);				// start from channel 0 ON
  Wire.send(0x00);				// set all ON time to 0
  Wire.send(0x00);
  Wire.send(lowByte(value0));	// set OFF according to value
  Wire.send(highByte(value0));
  Wire.send(0x00);
  Wire.send(0x00);
  Wire.send(lowByte(value1));
  Wire.send(highByte(value1));  
  Wire.send(0x00);
  Wire.send(0x00);
  Wire.send(lowByte(value2));
  Wire.send(highByte(value2));    
  Wire.endTransmission();
}
...
uint16_t HPRGB::pca9685GetPWM(uint8_t channel) 
{
  ...
  while(Wire.available()){
    uint8_t onLow = Wire.receive();
    uint8_t onHi = Wire.receive();
    uint8_t offLow = Wire.receive();
    uint8_t offHi = Wire.receive();
